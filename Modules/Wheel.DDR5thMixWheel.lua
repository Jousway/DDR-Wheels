...
local function MoveSelection(self,offset,Songs)
	...
	if type(Songs[CurSong]) ~= "string" then
		...
		DDR.CountingNumbers(self:GetChild("BPM_AFT"):GetChild("BPM"),self:GetChild("BPM_AFT"):GetChild("BPM"):GetText(),0,.1)
	end
	...
	return Def.ActorFrame{
		...
		Def.Sprite{
			...
			InitCommand=function(self)
				...
				self:zoom(DDR.Resize(self:GetWidth(),self:GetHeight(),60,60))
			...
			LoadCommand=function(self) 
				...
				self:zoom(DDR.Resize(self:GetWidth(),self:GetHeight(),60,60)):zoomy(0)
